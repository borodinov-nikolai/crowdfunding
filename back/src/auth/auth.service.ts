...
export class AuthService {
  ...
  async signUp(body: SignUpDto, res: Response) {
    ...
    const { accessToken, refreshToken } =
      await this.tokenService.generateTokens({ id: user.id, role: user.role });
  ...
  async refresh(req: Request, res: Response) {
    ...
    const compareResult = await this.tokenService.compareRefreshTokens({
      userId: payload.id,
      token: refreshToken,
    });
    ...
    res.cookie('jwt', freshRefreshToken, {
      httpOnly: true,
      maxAge: 30 * 24 * 60 * 60 * 1000,
    });
